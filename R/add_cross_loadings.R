...
add_cross_loadings <- function(
    lf_object,
    proportion_cross_loadings, proportion_cross_loadings_range = NULL,
    magnitude_cross_loadings, magnitude_cross_loadings_range = NULL,
    leave_cross_loadings = FALSE
)
{

  # Check inputs
  inputs <- add_cross_loadings_errors(
    lf_object,
    proportion_cross_loadings, proportion_cross_loadings_range,
    magnitude_cross_loadings, magnitude_cross_loadings_range
  )

  # Collect inputs
  parameters <- inputs$parameters
  integer_cross_loadings <- inputs$integer_cross_loadings
  ...
  cross_loading_list <- lapply(seq_along(start_cross_loadings), function(i){
    ...
    # Factor has no cross-loadings to place
    if(integer_cross_loadings[i] == 0){return(numeric(0))}
  ...
  for(i in factor_sequence){
    ...
    # Number of cross-loadings for this factor (empty when none are requested)
    cross_loading_sequence <- if(integer_cross_loadings[i] == 0){
      integer(0)
    }else{
      start_cross_loadings[i]:end_cross_loadings[i]
    }

    # Skip sampling and placement when this factor has no cross-loadings to place
    if(length(cross_loading_sequence) > 0){

      # Sample row
      random_row <- shuffle(
        seq_len(nrow(cross_loading_structure)),
        size = length(cross_loading_sequence)
      )

      # Sample column
      random_column <- shuffle_replace(
        seq_len(ncol(cross_loading_structure)),
        size = length(cross_loading_sequence)
      )

      # Loop through rows and columns
      for(j in seq_along(cross_loading_sequence)){
...
#' @noRd
# Updated 12.07.2026
add_cross_loadings_errors <- function(
    lf_object,
    proportion_cross_loadings, proportion_cross_loadings_range = NULL,
    magnitude_cross_loadings, magnitude_cross_loadings_range = NULL
)
{
  ...
  if(!is.null(proportion_cross_loadings_range)){
    ...
    proportion_cross_loadings <- runif_xoshiro(
      parameters$factors,
  ...
  if(!is.null(magnitude_cross_loadings_range)){
    ...
    magnitude_cross_loadings <- runif_xoshiro(
  ...
  return(
    list(
      ...
      integer_cross_loadings = integer_cross_loadings,
